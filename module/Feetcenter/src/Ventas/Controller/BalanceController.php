    ...
    public function filterAction()
    {
        ...
        if($request->isPost()){
            ...
            foreach ($clinicas as $clinica){
                ...
                $tmp['ingrsos_tarjeta'] = !is_null(\VisitapagoQuery::create()->filterByVisitapagoTipo('tarjeta')->useVisitaQuery()->filterByVisitaCreadaen(array('min' => $post_data['from'].' 00:00:00', 'max' => $post_data['to'].' 23:59:59'))->filterByIdclinica($clinica->getIdclinica())->withColumn('SUM(visitapago_cantidad)','total')->endUse()->select('total')->findOne()) ? \VisitapagoQuery::create()->filterByVisitapagoTipo('tarjeta')->useVisitaQuery()->filterByVisitaCreadaen(array('min' => $post_data['from'].' 00:00:00', 'max' => $post_data['to'].' 23:59:59'))->filterByIdclinica($clinica->getIdclinica())->withColumn('SUM(visitapago_cantidad)','total')->endUse()->select('total')->findOne() : 0;
