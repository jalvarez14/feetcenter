<?php



/**
 * This class defines the structure of the 'servicioinsumo' table.
 *
 *
 *
 * This map class is used by Propel to do runtime db structure discovery.
 * For example, the createSelectSql() method checks the type of a given column used in an
 * ORDER BY clause to know whether it needs to apply SQL to make the ORDER BY case-insensitive
 * (i.e. if it's a text column type).
 *
 * @package    propel.generator.feetcent_feetcenter.map
 */
class ServicioinsumoTableMap extends TableMap
{

    /**
     * The (dot-path) name of this class
     */
    const CLASS_NAME = 'feetcent_feetcenter.map.ServicioinsumoTableMap';

    /**
     * Initialize the table attributes, columns and validators
     * Relations are not initialized by this method since they are lazy loaded
     *
     * @return void
     * @throws PropelException
     */
    public function initialize()
    {
        // attributes
        $this->setName('servicioinsumo');
        $this->setPhpName('Servicioinsumo');
        $this->setClassname('Servicioinsumo');
        $this->setPackage('feetcent_feetcenter');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('idservicioinsumo', 'Idservicioinsumo', 'INTEGER', true, null, null);
        $this->addForeignKey('idservicio', 'Idservicio', 'INTEGER', 'servicio', 'idservicio', true, null, null);
        $this->addForeignKey('idinsumo', 'Idinsumo', 'INTEGER', 'insumo', 'idinsumo', true, null, null);
        $this->addColumn('servicioinsumo_cantidad', 'ServicioinsumoCantidad', 'DECIMAL', true, 10, null);
        // validators
    } // initialize()

    /**
     * Build the RelationMap objects for this table relationships
     */
    public function buildRelations()
    {
        $this->addRelation('Insumo', 'Insumo', RelationMap::MANY_TO_ONE, array('idinsumo' => 'idinsumo', ), 'CASCADE', 'CASCADE');
        $this->addRelation('Servicio', 'Servicio', RelationMap::MANY_TO_ONE, array('idservicio' => 'idservicio', ), 'CASCADE', 'CASCADE');
    } // buildRelations()

} // ServicioinsumoTableMap
